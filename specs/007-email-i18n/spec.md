...
- **One new targeted E2E.** A small `tests/e2e/email-i18n.spec.ts` asserts wiring (locale → catalog key → email subject) via Mailpit's HTTP API at `http://localhost:18025` — the smallest surface that closes Verification Gate 5 for the new user stories (added per `/speckit-analyze` finding C1). The existing `tests/e2e/auth.spec.ts` remains the regression net for the on-list magic-link flow itself. Manual Mailpit exercise (T011) discharges the Acceptance Scenarios for copy quality and visual layout, which an automated assertion cannot judge.
