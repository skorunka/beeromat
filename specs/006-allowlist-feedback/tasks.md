...
- [X] T008 Seven verification gates green: `pnpm typecheck`, `pnpm lint`, `pnpm test:unit` (21/21), `pnpm i18n:check`, `pnpm forms:check`, `pnpm build`, `pnpm test:e2e tests/e2e/auth.spec.ts` (1 passed, 48.8s). The on-list path regression net is intact; the contract supersession did not break the chain-link-3 auth fixture. (The full E2E suite was not run in this session — only the auth spec, which is the regression net for the changed surface. Other specs are unchanged behaviourally.)
...
- [X] T010 Promoted `specs/006-allowlist-feedback/spec.md` Status: `Shipped`. Merged `006-allowlist-feedback` into `main` with a merge commit. Push both branches.
