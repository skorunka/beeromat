...
- No E2E test of the sign-in form's not-on-allowlist path is added in v1.5 if the existing E2E covers the on-list path. A unit test of `requestMagicLinkAction`'s status output was attempted (T005) and deferred — the action's transitive imports (`@/lib/auth/session` → `next-intl` → `next/navigation`) don't resolve in the vitest environment without extending `vitest.config.ts` with path aliases or refactoring the action to extract a pure status-decision function. Both are larger than the test's marginal value at v1.5 scope. The status mapping is instead validated by `pnpm typecheck` (the new discriminator narrows correctly in `SignInForm.tsx`), the existing on-list sign-in E2E (which stays green), and manual exercise of the new screen.
